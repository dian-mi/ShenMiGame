...
class Engine:
    ...
    def next_turn(self):
        ...
        if len(alive) <= 3 and self.no_death_streak >= 2:
            target = alive[-1]
            self._log(f"【终局补刀】强制处决末位 {self.N(target)}")
...
class UI:
    def __init__(self, root: tk.Tk):
        ...
        # 字体
        self.font_rank = tkfont.Font(family="Microsoft YaHei UI", size=20, weight="normal")
        self.font_log  = tkfont.Font(family="Microsoft YaHei UI", size=20, weight="normal")
        self.font_log_bold = tkfont.Font(family="Microsoft YaHei UI", size=20, weight="bold")
...
欢迎大家游玩
    ...
    def _build(self):
        ...
        # 两栏显示：左侧排名区分成2列（13行×2列）
        self.rank_frame.grid_columnconfigure(0, weight=1)
        self.rank_frame.grid_columnconfigure(1, weight=1)

        for rr in range(13):
            self.rank_frame.grid_rowconfigure(rr, weight=0)
        self.rank_frame.grid_rowconfigure(13, weight=1)  # 吃掉多余高度的 spacer 行
        ...
        for i in range(26):
            ...
            r = i % 13
            c = i // 13
            row.grid(row=r, column=c, sticky="ew", padx=(0, 8) if c == 0 else (8, 0), pady=1)
