...
class Status:
    ...
    pursuit_kill_used: int = 0        # 追击斩杀已用次数：施博理
...
class Engine:
    ...
    def __init__(self, seed: Optional[int] = None, fast_mode: bool = False):
        ...
        # Seed feature flag (UI controlled). When enabled, we will log the effective seed at game start/end.
        self.seed_enabled: bool = False
        # The effective seed actually used for this game (may be randomly generated when base_seed is None).
        self.current_seed: Optional[int] = seed
    ...
    def new_game(self):
        ...
        try:
            ...
            pass
        self.turn = 0
        self.world_event_triggered_this_turn = False  # 本回合是否触发世界事件        # RNG reset policy (with optional Seed feature):
        # - If seed_enabled is False: always use non-deterministic randomness per new_game; do not log seed.
        # - If seed_enabled is True:
        #   * base_seed is None => generate a random seed for THIS game, store as current_seed, and use it.
        #   * base_seed is set  => use it as current_seed, deterministic across new_game.
        if not getattr(self, 'seed_enabled', False):
            self.current_seed = None
            self.rng = random.Random()
        else:
            if self.base_seed is None:
                # generate a reproducible seed for this game while still being random each new_game
                self.current_seed = random.SystemRandom().randint(0, 2**31 - 1)
        ...
        self.pending_endgame_execute = False
        self.log = []
        self.replay_frames = []        # Seed banner: log effective seed only when seed feature is enabled
        if getattr(self, 'seed_enabled', False):
            if getattr(self, 'current_seed', None) is None:
                self._log("【Seed】本局seed=0")
            else:
                self._log(f"【Seed】本局seed={self.current_seed}")
    ...
    def next_turn(self):
        ...
        for _cid, _r in self.roles.items():
            st = _r.status
            if hasattr(st, "purify_ttl") and st.purify_ttl > 0:
                st.purify_ttl -= 1
                if st.purify_ttl <= 0:
                    st.purify_ttl = 0
        # ---------------------------------------------------------------
        ...
        if not alive:
            if getattr(self, 'seed_enabled', False):
                self._log(f"【Seed】本局seed={self.current_seed if self.current_seed is not None else 0}")
            self.game_over = True
            return
        if len(alive) > 3:
            self.pending_endgame_execute = False
        ...
        for cid in self.alive_ids():
            ...
            if cid == 45 and (not st.perma_disabled):
                ...
                if sr is not None and cr is not None and (cr - sr) >= 2 and getattr(st, "defense_line_ttl", 0) == 0:
                    ...
                    self._log(f"  · 锁定防线：{self.N(45)} 获得【防线】(5回合)")
            ...
            if getattr(st, "defense_line_ttl", 0) > 0:
                self.move_by(cid, -2, source=None, note="防线上升2名")
...
# 辩护：持续期间每回合上升1名
            ...
            # 乘胜追击：每层乘胜追击=每回合上升对应名次（1层+1，2层+2，3层+3），登顶清空
            if getattr(st, "chase", 0) > 0:
                self.move_by(cid, -st.chase, source=None, note=f"乘胜追击上升{st.chase}名")
                # 冷雨霏：到达第一名时清空乘胜追击层数
                if self.rank_no(cid) == 1 and st.chase > 0:
                    self._log(f"  · 乘胜追击：{self.N(cid)} 登顶，清空层数")
                    st.chase = 0
        ...
        if died:
            ...
            if higher2:
                ...
                if t2 is not None and (not self.is_mls_unselectable_by_active_kill(t2)):
                    ...
                    used = self.roles[15].mem.get('extra_kill_used', 0)

                    if used >= 2:

                        self._log(f"  · 追加清算：{self.N(15)} 已达上限(2次)，本局不再发动")
                        ...
                        self.roles[15].mem['extra_kill_used'] = used + 1
    ...
    def act_16(self):
        ...
        if myr is None:
            return
        if myr <= int(len(alive) * 0.7):
            self._log("  · 众意审判：不在后30%，条件不满足")
            return
        first = alive[0]
        tail = alive[int(len(alive) * 0.7):]
    ...
    def act_25(self):
        ...
        for i in range(10):
            ...
            self._log(f"    - 第{i+1}次：随机斩杀 {self.N(v)}")
            # Direct elimination (bypass shields & revival) to match the joke-mode description.
            self.kill(v, 25, "玩笑模式随机斩杀", bypass_shield=True, bypass_revive=True)
...
class UI:
    ...
    def _log_ui_toggle(self, name: str, enabled: bool):
        try:
            state = "已打开" if enabled else "已关闭"
            self.engine._log(f"【设置】{name}{state}")
    ...
    def __init__(self, root: tk.Tk):
        ...
        # Seed UI: feature flag + seed value.
        self.seed_enabled_var = tk.BooleanVar(value=False)
        self.seed_var = tk.StringVar(value="")
        # Sync to engine (disabled by default)
        self.engine.seed_enabled = bool(self.seed_enabled_var.get())
        # 播放/回放状态
        self.play_cursor = 0
        self.playing = False
        ...
        self.preserve_history.trace_add("write", lambda *_: self._log_ui_toggle("保留历史记录", self.preserve_history.get()))
        ...
        # When rapidly advancing turns, flush remaining per-turn logs instead of skipping them.
        self.keep_skipped_logs = tk.BooleanVar(value=True)
        self.keep_skipped_logs.trace_add("write", lambda *_: self._log_ui_toggle("跳过回合时保留日志输出", self.keep_skipped_logs.get()))
        self.export_error_log = False
    ...
    def _build_menu(self):
        ...
        menu_help.add_command(label="说明", command=self.show_help)
        menubar.add_cascade(label="说明", menu=menu_help)

        # === 统计 ===
        menu_stats = tk.Menu(menubar, tearoff=0)
        menu_stats.add_command(label="快速跑500局", command=self.on_sim_500)
        menu_stats.add_command(label="快速跑5000局", command=self.on_sim_5000)
        menu_stats.add_command(label="快速跑50000局", command=self.on_sim_50000)
        menubar.add_cascade(label="统计", menu=menu_stats)

        # === 自定义 ===
        menu_custom = tk.Menu(menubar, tearoff=0)
        self.menu_custom = menu_custom
        # Seed menu item is conditionally shown based on Settings > 启用种子
        self._refresh_seed_menu_item()
        menu_custom.add_checkbutton(label="找自称无敌模式", variable=self.joke_mode, command=self._on_toggle_joke_mode)
        menubar.add_cascade(label="自定义", menu=menu_custom)

        # === 设置 ===
        menu_settings = tk.Menu(menubar, tearoff=0)
        menu_settings.add_checkbutton(label="启用种子", variable=self.seed_enabled_var, command=self._on_toggle_seed_enabled)
        menu_settings.add_checkbutton(
            label="输出异常日志到脚本目录(error_log.txt)",
            variable=self.export_error_log,
            command=self._on_toggle_export_error,
        )
        menu_settings.add_separator()
        menu_settings.add_checkbutton(
            label="自动跳过回合（回合结束后5秒）",
            variable=self.auto_skip_turn,
            command=self._on_toggle_auto_skip,
        )
        menu_settings.add_checkbutton(
            label="跳过回合时保留日志输出",
            variable=self.keep_skipped_logs,
        )
        menu_settings.add_checkbutton(label="保留历史记录", variable=self.preserve_history)
        menu_settings.add_checkbutton(
            label="显示实名",
            variable=self.show_realname,
            command=self._on_toggle_show_realname,
        )
        menu_settings.add_checkbutton(
            label="显示首字母",
            variable=self.show_initials,
            command=self._on_toggle_show_initials,
        )
        menu_settings.add_separator()
        menu_settings.add_command(label="字体放大", command=lambda: self.adjust_font(2))
        menu_settings.add_command(label="字体缩小", command=lambda: self.adjust_font(-2))
    ...
    def _on_toggle_export_error(self):
        ...
        try:
            ...
            pass
        self._log_ui_toggle("输出异常日志", self.export_error_log.get())
    ...
    def _on_toggle_auto_skip(self):
        ...
        # If the user advances to the next turn before the current turn log has finished playing,
        # optionally flush the remaining log lines so they are not skipped.
        if self.keep_skipped_logs.get():
            try:
                frames = self.engine.replay_frames
                if frames and self.play_cursor < len(frames):
                    for k in range(self.play_cursor, len(frames)):
                        fr = frames[k]
                        self.revealed_lines.append(fr["text"])
                        self.revealed_hls.append(fr.get("highlights", []))
                        self.revealed_victims.append(self._parse_victim_cid(fr["text"]))
                    self.play_cursor = len(frames)
                    last = frames[-1]
                    self.current_snap = last.get("snap")
                    self.current_highlights = set(last.get("highlights", []))
                    self.refresh_replay_view()
            except Exception:
                pass
        self._log_ui_toggle("自动跳过回合", self.auto_skip_turn.get())
    ...
    def open_seed_dialog(self):
        """Open a small dialog to set the per-game seed.
        - Empty/None => fully random each new game
        - Integer => deterministic across new_game (same seed)
        """
        ...
        ttk.Label(frm, text="Seed（留空=完全随机；填整数=可复现）").grid(row=0, column=0, sticky="w")
        ent = ttk.Entry(frm, textvariable=self.seed_var, width=30)
        ent.grid(row=1, column=0, sticky="w", pady=(8, 0))
        ent.focus_set()

        # Show current effective seed
        cur = "None（完全随机）" if self.engine.base_seed is None else str(self.engine.base_seed)
        ttk.Label(frm, text=f"当前：{cur}").grid(row=2, column=0, sticky="w", pady=(8, 0))

        btns = ttk.Frame(frm)
        btns.grid(row=3, column=0, sticky="e", pady=(14, 0))
        ttk.Button(btns, text="取消", command=win.destroy).grid(row=0, column=0, padx=(0, 8))
        ttk.Button(btns, text="应用并新开局", command=lambda: self._apply_seed_and_restart(win)).grid(row=0, column=1)

        def _on_enter(_evt):
            self._apply_seed_and_restart(win)

        ent.bind("<Return>", _on_enter)

    def _apply_seed_and_restart(self, dialog: Optional[tk.Toplevel] = None):
        raw = (self.seed_var.get() or "").strip()
        if raw == "" or raw.lower() in ("none", "random", "rand"):
            new_seed = None
        else:
            try:
                new_seed = int(raw)
                ...
                try:
                    messagebox.showerror("Seed无效", "Seed必须是整数，或留空表示随机。")
                except Exception:
                    pass
                return

        # Update engine seed policy
        self.engine.seed_enabled = True
        self.engine.base_seed = new_seed

        # Close dialog first (avoid focus issues)
        try:
    ...
    # ---------- UI 构建 ----------
        self._log_ui_toggle("找自称无敌模式", self.joke_mode.get())



    def _refresh_seed_menu_item(self):
        """Show '设置Seed…' under 自定义 only when seed feature is enabled."""
        if not hasattr(self, "menu_custom") or self.menu_custom is None:
            return
        label = "设置Seed…"
        # Check if item exists
        exists = False
        try:
            end = self.menu_custom.index("end")
            if end is not None:
                for i in range(end + 1):
                    try:
                        if self.menu_custom.type(i) == "command" and self.menu_custom.entrycget(i, "label") == label:
                            exists = True
                            idx = i
                            break
                    except Exception:
        ...
        if self.seed_enabled_var.get():
            if not exists:
                try:
                    self.menu_custom.insert_command(0, label=label, command=self.open_seed_dialog)
                except Exception:
                    # Fallback: append
                    try:
                        self.menu_custom.add_command(label=label, command=self.open_seed_dialog)
                    except Exception:
                        pass
        else:
            if exists:
                try:
                    self.menu_custom.delete(idx)
                except Exception:
                    pass

    def _on_toggle_seed_enabled(self):
        """Settings > enable/disable seed feature."""
        enabled = bool(self.seed_enabled_var.get())
        # Sync to engine
        self.engine.seed_enabled = enabled
        if not enabled:
            # When disabled, we keep seed_var as-is but engine will ignore it.
            self.engine.current_seed = None
        self._refresh_seed_menu_item()
    ...
    def _on_toggle_show_realname(self):
        ...
        self.refresh()
        self._log_ui_toggle("显示实名", self.show_realname.get())
    ...
    def _on_toggle_show_initials(self):
        ...
        self.refresh()
        self._log_ui_toggle("显示首字母", self.show_initials.get())
...
但是其实都是GPT大人神力
欢迎大家游玩
网页版链接：https://shenmigame-hv9nrxdcmvwe3ctyevjjue.streamlit.app/
"""
    ...
    def _run_quick_sim(self, GAMES: int):
        try:
            self.log_text.configure(state="normal")
            self.log_text.insert(tk.END, f"\n【测试】开始快速模拟{GAMES}局…（期间界面可能短暂无响应，请不要退出）\n")
            self.log_text.configure(state="disabled")
            self.log_text.see(tk.END)
    ...
    def render_log_with_current_highlight(self, lines: List[str], hls: List[List[int]]):
        ...
        self.log_text.tag_configure("dot_gray", foreground="#444444")
        try:
            # Ensure dot_gray is low priority so red/bold highlights remain intact.
            self.log_text.tag_lower("dot_gray")
        except Exception:
            pass
        last_i = len(lines) - 1
        for i, line in enumerate(lines):
            ...
            # Lines starting with "·" (after stripping leading spaces) are shown in deep gray.
            if disp_line.lstrip().startswith("·"):
                self.log_text.tag_add("dot_gray", start_idx, end_idx)
            if "触发随机事件：" in line2:
