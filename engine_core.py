...
class Engine:
    ...
    def kill(self, victim: int, killer: Optional[int], reason: str,
        ...
        if victim not in self.roles or not self.roles[victim].alive:
            return False
        # Streamlit helper: generic invincible toggle
        if victim in self.__dict__.get('_invincible_cids', set()):
            self._log(f"  · 无敌：{self.N(victim)} 免疫本次淘汰（{reason}）")
            return False
...
# =========================
# Streamlit / Headless helpers
# =========================
def _engine_set_invincible(self, cid: int, enable: bool = True):
    """Mark a role as invincible (immune to kill), used by Streamlit UI toggles/tests."""
    s = self.__dict__.setdefault("_invincible_cids", set())
    if enable:
        s.add(cid)
    else:
        s.discard(cid)
